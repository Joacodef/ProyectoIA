    ...
    while(!finEjecucion){
        ...
        if(clientesVisitados.listSize >= abs(inst.numClientes)) finEjecucion = true;
            ...
            if(backTracking){
                ...
                clientesVisitados.moveToStart();
                for(unsigned int i=0;i<vehiculoActual->recorrido.listSize-2;i++){
            ...
            while(eficienciaReco < eficienciaEsperada && numIntentos < MAX_INTENTOS){
                ...
                cout << "concatenacion de ambas: "<< visitadosYRestringidos.to_string()<<"\n";
                ...
                if(eficienciaReco < eficienciaEsperada){
                    ...
                    if(clientesVisitados.listSize + clientesRestringidosTemp.listSize < abs(inst.numClientes)){
        ...
        if(eficienciaReco < eficienciaEsperada){
            ...
            clientesRestringidosTemp.clear();
            clientesVisitados.moveToStart();
            for(unsigned int i=0;i<vehiculoActual->recorrido.listSize;i++){
                clientesVisitados.remove();
            }
            vehiculoActual->reiniciarRecorrido();
            vehiculos.moveToEnd();
...
/*void quitarNodosRestringidosTemp(ListaNodos nodosRestringidos){
    nodosRestringidos.moveToStart();
    nodosRestringidos.next();
    for(unsigned int i = 0;i<nodosRestringidos.listSize+1;i++){
        if(nodosRestringidos.curr){
            if(nodosRestringidos.curr->data.restringidoTemp){
                nodosRestringidos.prev();
                nodosRestringidos.remove();
            }
        }
        else{
            break;
